...
void game_init(void) {
  ...
  score = 0;
  lcd_show_string(20, 230, "Snake (use buttons)", TEXT_COLOR, BACKGROUND_COLOR,
                  16, 0);
  draw_score();
...
static void draw_score(void) {
  ...
  lcd_draw_rectangle(230, 20, 270, 200,
                     BACKGROUND_COLOR); /* adapt coords if needed */
  lcd_show_string(20, 250, buf, TEXT_COLOR, BACKGROUND_COLOR, 16, 0);
}

static void restart_game(void) {
  /* small restart procedure: clear maze and re-init */
  lcd_show_string(20, 260, "Game over. Restarting...", TEXT_COLOR,
                  BACKGROUND_COLOR, 12, 0);
